...
angular.module("managerApp").controller("rapeListCtrl", ["$scope", "$http", function($scope, $http) {
  ...
  //Es la forma antigua de hacer las búsquedas
...
//El offset ha sido quitado en esta versión, ya que no es nada útil, con el limit es suficiente, y el offset será siempre 0

  $scope.pagination = function(limit) {
    console.log("Hemos entrado en el pagination");
    $scope.offset = 0;
    $scope.currentPage = 1;
    var pages = [];
    if ($scope.limit > 0) {
      $http
        ...
        .then(function(response) {
          ...
          for (var i = 1; i <= ((response.data.length ) / $scope.limit); i++) {
            pages.push(i);

          }
          if (pages.length * $scope.limit < response.data.length) {
            pages.push(pages.length + 1);
          }
          $scope.pages = pages;
          document.getElementById("numberPagination").disabled = false;
